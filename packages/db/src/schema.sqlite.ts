...
export const profiles = sqliteTable('profiles', {
  ...
  // Pasos 2 y 3
  objetivos: text('objetivos', { mode: 'json' }).$type<Objetivo[]>().notNull().default([]),
  ...
  edadDeclaradaEn: text('edad_declarada_en'),

  // Paso 7
  gymPrincipalId: text('gym_principal_id').references(() => gyms.id, { onDelete: 'set null' }),

  // Paso 9
  comidasPorDia: integer('comidas_por_dia'),
  desayuna: integer('desayuna', { mode: 'boolean' }),
  ayunoIntermitente: integer('ayuno_intermitente', { mode: 'boolean' }),
  ...
  suplementos: text('suplementos', { mode: 'json' }).$type<string[]>().notNull().default([]),
  ...
  duermeConReloj: integer('duerme_con_reloj', { mode: 'boolean' }),
  ...
  onboardingCompletadoEn: integer('onboarding_completado_en', { mode: 'timestamp_ms' }),
...
export const userFoods = sqliteTable(
  'user_foods',
  {
    ...
    habitual: integer('habitual', { mode: 'boolean' }).notNull().default(true),
    creadoEn,
    actualizadoEn,
  },
  (t) => [
